...
def generate_launch_description():
    ...
    camera_ip_arg = DeclareLaunchArgument(
        'camera_ip', default_value='10.0.2.11', description='IP address of the Lucid camera'
    )

    topic_name_arg = DeclareLaunchArgument(
        'topic_name', default_value='/image_raw', description='Topic name for publishing images'
    )

    horizontal_binning_arg = DeclareLaunchArgument(
        'horizontal_binning', default_value='2', description='Horizontal binning factor'
    )

    vertical_binning_arg = DeclareLaunchArgument(
        'vertical_binning', default_value='2', description='Vertical binning factor'
    )

    vpi_backend_arg = DeclareLaunchArgument(
        'vpi_backend', default_value='cuda', description='VPI backend to use (cuda, vic, or cpu)'
    )
