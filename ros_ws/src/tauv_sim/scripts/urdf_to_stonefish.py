...
class RobotConfig:
    ...
    base_link_mass_override: Optional[float]
    include_arm: bool
...
class StonefishGenerator:
    ...
    def _add_arm(self, robot: ET.Element, urdf_parser: URDFParser):
        ...
        if arm_info['arm_base_joint'] is not None:
            self._add_arm_joint(robot, arm_info['arm_base_joint'], 'os/arm/base', 'os_hull', 'ArmLink')
            
        # Add arm link joint (connects ArmLink to ArmTube)
        if arm_info['arm_link_joint'] is not None:
            self._add_arm_joint(robot, arm_info['arm_link_joint'], 'os/arm/link', 'ArmLink', 'ArmTube')
        
        # Add arm actuators
        self._add_arm_actuators(robot)
    
    def _add_arm_link(self, robot: ET.Element, link_name: str, urdf_link_name: str, visual_transform: Optional[Transform] = None, urdf_parser: Optional[URDFParser] = None):
        ...
        if urdf_parser:
            ...
            if arm_inertial and arm_inertial.is_valid():
                ...
                inertia.set('xyz', f"{arm_inertial.inertia[0]} {arm_inertial.inertia[1]} {arm_inertial.inertia[2]}")
                
                cg = ET.SubElement(link, 'cg')
                cg.set('xyz', f"{arm_inertial.cg_xyz[0]} {arm_inertial.cg_xyz[1]} {arm_inertial.cg_xyz[2]}")
                cg.set('rpy', f"{arm_inertial.cg_rpy[0]} {arm_inertial.cg_rpy[1]} {arm_inertial.cg_rpy[2]}")
            else:
    ...
    def _add_arm_joint(self, robot: ET.Element, urdf_joint: ET.Element, joint_name: str, parent_name: str, child_name: str):
...
def main():
    ...
    if not args.config and not os.path.exists(config_path):
        print(f"Note: No configuration file found. Use --create-config to generate a default one.")
