...
class OnshapeConfig:
    ...
    def create_default_config(self):
        ...
        default_config = {
            ...
            'mesh_downsampling': {
                ...
                'per_mesh_limits': {
                    'os_hull.stl': {'max_faces': 10000},
                    'thruster.stl': {'max_faces': 1000},
                    'os_arm_base.stl': {'max_faces': 2000},
                    'os_arm_link.stl': {'max_faces': 1500},
                    'os_arm_tube.stl': {'max_faces': 1500},
                    'os_launcher.stl': {'max_faces': 3000},
                    'os_dropper.stl': {'max_faces': 2000},
                },
            },
            'output': {'scenario_file': 'osprey.scn', 'overwrite_existing': True},
        }
    ...
    def get_environment_dict(self) -> dict[str, str]:
        ...
        print("Using Onshape environment variables:")
        print(f"  ONSHAPE_API = {self.api_url}")
        print(
            f"  ONSHAPE_ACCESS_KEY = {self.access_key[:8]}..."
            if self.access_key
            else "  ONSHAPE_ACCESS_KEY = (not set)"
        )
        print(
            f"  ONSHAPE_SECRET_KEY = {self.secret_key[:8]}..."
            if self.secret_key
            else "  ONSHAPE_SECRET_KEY = (not set)"
        )
...
class MeshProcessor:
    ...
    def downsample_mesh(self, mesh_path: str) -> bool:
        ...
        try:
            ...
            print(
                f"  Original: {ms.current_mesh().vertex_number()} vertices, {ms.current_mesh().face_number()} faces"
            )
            ...
            if max_faces < current_faces:
                ...
                print(
                    f"  Downsampled: {ms.current_mesh().vertex_number()} vertices, {ms.current_mesh().face_number()} faces"
                )
            ...
            ms.save_current_mesh(mesh_path, binary=False)  # ASCII format
...
class OnshapePipeline:
    ...
    def run_urdf_to_stonefish(
        self, urdf_path: str, output_path: str, stonefish_config: str
    ) -> bool:
