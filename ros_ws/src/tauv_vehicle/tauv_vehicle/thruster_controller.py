...
class ThrusterController(Node):
    ...
    def _thrust_to_omega(self, thrust_n: float) -> float:
        ...
        omega_radps = np.sqrt(min(abs(thrust_n) / thrust_coeff, MAX_SETPOINT**2)) * np.sign(
            thrust_n
        )
