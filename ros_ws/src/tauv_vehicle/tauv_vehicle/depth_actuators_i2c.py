...
from tauv_msgs.msg import DepthSensorFrame, ThrusterSetpoint

from .i2c_drivers import DENSITY_FRESHWATER, UnifiedI2CDriver, UNITS_Centigrade, UNITS_Pa
...
class DepthActuatorsI2C(Node):
    ...
    def __init__(self):
        ...
        self._driver = UnifiedI2CDriver(bus_number=I2C_BUS, pca9685_address=PCA9685_ADDRESS)
        ...
        self._thruster_setpoint_sub = self.create_subscription(
            ThrusterSetpoint, "thruster_setpoint", self._handle_thruster_setpoint, 10
        )

        # Publisher for depth sensor data
        self._depth_sensor_pub = self.create_publisher(DepthSensorFrame, "depth", 10)

        # Timer for periodic depth sensor reading
        if self._ms5837_ok:
            self._depth_timer = self.create_timer(1.0 / depth_sensor_rate, self._read_depth_sensor)
