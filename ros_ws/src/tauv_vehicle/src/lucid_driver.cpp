...
private:
    Arena::IDevice* getDeviceByIP(const std::string& ip)
    {
        ...
        {
            RCLCPP_INFO(this->get_logger(), 
                        "Try %d of 6: waiting for 10 secs for device to be connected!", 
                        tries + 1);
    ...
    void setupCamera()
    {
        ...
        Arena::SetNodeValue<GenICam::gcstring>(stream_nodemap, "StreamBufferHandlingMode", "NewestOnly");
    ...
    void streamImages()
    {
        ...
        while (rclcpp::ok())
        {
            ...
            try {
                ...
                auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(curr_time - prev_time);
                double fps = 1000.0 / duration.count();
                RCLCPP_INFO(this->get_logger(), "FPS: %.2f", fps);
                RCLCPP_INFO(this->get_logger(), "Image Size (%d, %d)", 
    ...
    void initializeVPI(int width, int height)
    {
        try {
            ...
            // Scaled NV12 image
            CHECK_VPI_STATUS(vpiImageCreate(output_width_, output_height_, VPI_IMAGE_FORMAT_NV12_ER, 0, &vpi_scaled_nv12_));
            
            // Output RGB8 image
            CHECK_VPI_STATUS(vpiImageCreate(output_width_, output_height_, VPI_IMAGE_FORMAT_RGB8, 0, &vpi_output_));
            
            RCLCPP_INFO(this->get_logger(), 
                       "VPI initialized: %dx%d -> %dx%d, Backend: %s", 
                       width, height, output_width_, output_height_,
    ...
    cv::Mat processWithVPI(const cv::Mat& input)
    {
        try {
            ...
            CHECK_VPI_STATUS(vpiImageCreateWrapperOpenCVMat(input, VPI_IMAGE_FORMAT_BGR8, 0, &vpi_input_));
            
            // Convert BGR8 to NV12_ER using CUDA backend for efficiency
            CHECK_VPI_STATUS(vpiSubmitConvertImageFormat(vpi_stream_, VPI_BACKEND_CUDA, 
                                                         vpi_input_, vpi_temp_nv12_, nullptr));
            
            // Rescale the image using the selected backend
            CHECK_VPI_STATUS(vpiSubmitRescale(vpi_stream_, vpi_backend_, 
                                              vpi_temp_nv12_, vpi_scaled_nv12_, 
                                              VPI_INTERP_LINEAR, VPI_BORDER_CLAMP, 0));
            
            // Convert back to RGB8 using CUDA
            CHECK_VPI_STATUS(vpiSubmitConvertImageFormat(vpi_stream_, VPI_BACKEND_CUDA, 
                                                         vpi_scaled_nv12_, vpi_output_, nullptr));
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("lucid_driver"), 
