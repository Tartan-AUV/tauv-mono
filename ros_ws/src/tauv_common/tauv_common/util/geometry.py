...
import numpy as np
from geometry_msgs.msg import Pose, Quaternion, Transform, Twist, Vector3, Wrench
from spatialmath import SE3, SO3, UnitQuaternion


def _validate_finite_values(values: list | tuple | np.ndarray, name: str) -> None:
...
def numpify(
    obj: Vector3 | Quaternion | Transform | Wrench | Twist,
) -> np.ndarray | UnitQuaternion | SE3 | np.ndarray:
...
@overload
def msgify(
    obj: np.ndarray, *, message_type: str = None
) -> Vector3 | Quaternion | Wrench | Twist: ...
...
def msgify(
    obj: np.ndarray | UnitQuaternion | SE3 | SO3 | np.ndarray, *, message_type: str = None
) -> Vector3 | Quaternion | Transform | Wrench | Twist:
    ...
    if isinstance(obj, np.ndarray):
        ...
        return Quaternion(
            x=float(obj.vec[0]), y=float(obj.vec[1]), z=float(obj.vec[2]), w=float(obj.s)
        )
