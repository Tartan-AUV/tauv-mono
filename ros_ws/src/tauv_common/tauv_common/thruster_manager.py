...
class ThrusterManager(Node):
    def __init__(self):
        ...
        self._wrench_sub = self.create_subscription(
            WrenchStamped, 'target_wrench', self._handle_wrench, 10
        )
        self._target_thrust_pub = self.create_publisher(TargetThrust, 'target_thrust', 10)

        self._thruster_frames = [
            f"{TF_NAMESPACE}/thruster/{thruster_id}" for thruster_id in THRUSTER_IDS
        ]
        ...
        for i, thruster_frame in enumerate(self._thruster_frames):
            try:
                transform = self._tf_buffer.lookup_transform(
                    wrench_frame, thruster_frame, self.get_clock().now(), Duration(seconds=0)
                )
                W_T_T[i] = numpify(transform.transform)
            except Exception as e:
                self.get_logger().error(
                    f"Could not get transform from {wrench_frame} to {thruster_frame}: {e}"
                )
                return
