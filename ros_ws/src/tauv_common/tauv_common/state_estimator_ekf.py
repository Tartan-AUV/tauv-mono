...
from tauv_msgs.msg import Depth as DepthMsg, NavigationState, WaterlinkedDvlFrame as DvlMsg
...
class EkfHistory:
    ...
    def __init__(
        ...
        self.state_history: dict[
            int, tuple[MeasurementType, DepthInput | DvlInput, np.ndarray, np.ndarray]
        ] = {}
    ...
    def add_imu_measurement(self, t: int, imu: EkfControl) -> None:
        ...
        if t <= self.last_imu_t:
            raise ValueError(
                f"IMU measurement at {t} is not newer than last IMU at {self.last_imu_t}"
            )

        if t <= self.last_dvl_t:
            self._logger.warning(
                f"IMU measurement at {t} is not newer than last DVL at {self.last_dvl_t}"
            )
            return
    ...
    def add_depth_measurement(
        self, t: int, depth: DepthInput, ekf: 'Ekf', local_T_odom: SE3
    ) -> None:
        ...
        if t <= self.last_dvl_t:
            self._logger.warning(
                f"Depth measurement at {t} is not newer than last DVL at {self.last_dvl_t}"
            )
            return
            # TODO: Handle this case
    ...
    def add_dvl_measurement(self, t: int, dvl: DvlInput, ekf: 'Ekf', local_T_odom: SE3) -> None:
        ...
        dvl_times = [
            tt
            for tt, (mtype, _, _, _) in self.state_history.items()
            if mtype == MeasurementType.DVL
        ]
        if any(t <= tt for tt in dvl_times):
            logging.warning(
                f"DVL measurement at {t} is not newer than existing DVL measurements. Rejecting."
            )
        ...
        for replay_t in subsequent_times:
            ...
            if mtype == MeasurementType.DVL:
                ...
                raise ValueError(
                    f"Encountered another DVL measurement at {replay_t} during replay. DVL measurements must be in order."
                )
                ...
                P_pred = ekf.predict_cov(
                    latest_est[3], closest_control_data, dt_replay, local_T_odom
                )
                ...
                # Apply depth update
                x_updated_replay, P_updated_replay = ekf.update(
                    x_pred, P_pred, z, R, z_pred, ekf.H_depth
                )

                # Update the state at replay_t
                assert x_updated_replay.shape == (6, 1)
                assert P_updated_replay.shape == (6, 6)
                self.state_history[replay_t] = (
    ...
    def _find_latest_state_before(
        self, t: int
    ) -> tuple[int, tuple[MeasurementType, DepthInput | DvlInput, np.ndarray, np.ndarray]]:
    ...
    def get_latest_state(
        self,
    ) -> tuple[int, tuple[MeasurementType, DepthInput | DvlInput, np.ndarray, np.ndarray]] | None:
    ...
    def _cleanup(self) -> None:
        ...
        if self.control_history:
            ...
            self.state_history = {
                k: v for k, v in self.state_history.items() if k >= oldest_control_time
            }
...
class Ekf:
    ...
    def predict(self, xkm1: np.ndarray, uk: EkfControl, dt: int, local_T_odom: SE3) -> np.ndarray:
        ...
        a_body_B = body_R_imu * uk.a_S - np.cross(
            omega_B_flat, np.cross(omega_B_flat, r_sensor__body_B)
        ).reshape(3, 1)
    ...
    def predict_cov(
        self, Pkm1: np.ndarray, uk: EkfControl, dt: int, local_T_odom: SE3
    ) -> np.ndarray:
        """Predict covariance"""
        ...
        F = np.block([[np.eye(3), odom_R_body.A * dt_seconds], [np.zeros((3, 3)), np.eye(3)]])
...
class StateEstimatorEkf(Node):
    def __init__(self) -> None:
        ...
        self.initial_position_stddev_m = self.declare_parameter(
            'initial_position_stddev_m', 0.01
        ).value
        self.initial_velocity_stddev_mps = self.declare_parameter(
            'initial_velocity_stddev_mps', 0.1
        ).value
        self.process_noise_density_pos = self.declare_parameter(
            'process_noise_density_pos_m_per_sqrt_s', 0.01
        ).value
        self.process_noise_density_vel = self.declare_parameter(
            'process_noise_density_vel_mps_per_sqrt_s', 0.01
        ).value
        ...
        self.depth_sub: Subscription | None = None
        self.dvl_sub: Subscription | None = None
        self.nav_state_pub: Publisher | None = None
        self.processing_thread: Thread | None = None

        # State
        self.ekf: Ekf | None = None
        self.history: EkfHistory | None = None
        self.static_transforms: EkfStaticTransforms | None = None
        ...
        self.retare_service = self.create_service(
            Empty, 'retare_local_frame', self._retare_callback
        )
    ...
    def _static_tf_timer_callback(self) -> None:
        ...
        self.get_logger().debug(
            f"Looking up transforms: {self.body_frame}, {self.depth_frame}, {self.dvl_frame}"
        )
        try:
            now = self.get_clock().now()
            # Look up depth transform
            depth_tf = self.tf_buffer.lookup_transform(self.body_frame, self.depth_frame, now)
            # Look up DVL transform
            dvl_tf = self.tf_buffer.lookup_transform(self.body_frame, self.dvl_frame, now)
            # Look up IMU transform
            imu_tf = self.tf_buffer.lookup_transform(self.body_frame, self.imu_frame, now)
            # Extract transforms
    ...
    def _processing_loop(self) -> None:
        ...
        while rclpy.ok():
            try:
                ...
                if depth_input_stamped is not None and imu_input_stamped is not None:
                    break
        ...
        try:
            self.history = EkfHistory(
                t_depth, t_imu, depth_input, imu_input, self.params, 50, self.get_logger()
            )
            self.get_logger().info(
                "EKF history initialized without DVL - ready to publish navigation state"
            )
    ...
    def _publish_navigation_state(self) -> None:
        ...
        nav_msg.header.stamp = TimeMsg(
            sec=latest_imu_t_ns // 1_000_000_000, nanosec=latest_imu_t_ns % 1_000_000_000
        )
