...
namespace sf
{
    ...
    public:
        ...
        // Publishes pressure as tauv DepthSensorFrame (replaces FluidPressure adapter)
        void PublishPressure(rclcpp::PublisherBase::SharedPtr pub, Pressure* press) const;
        // Publishes DVL as tauv WaterlinkedDvlFrame (replaces stonefish_ros2::DVL adapter)
    ...
    private:
        ...
        // Track last publish time per DVL to populate Waterlinked message 'time' delta
        mutable std::unordered_map<std::string, rclcpp::Time> last_dvl_stamp_;
        // External temperature parameter used for DepthSensorFrame
        double external_temperature_ {25.0};
    };
}
#endif
