...
void ROS2SimulationManager::ThrustersTauvCallback(const tauv_msgs::msg::ThrusterSetpoint::SharedPtr msg, std::shared_ptr<ROS2Robot> robot)
{
    // Validate size
    if(msg->omega_radps.size() != robot->thrusterSetpoints_.size())
    {
        RCLCPP_ERROR_STREAM(nh_->get_logger(), "Wrong number of tauv thruster setpoints for robot: " << robot->robot_->getName()
            << ", expected " << robot->thrusterSetpoints_.size() << ", got " << msg->omega_radps.size());
        return;
    }

    // Update setpoints. Disabled channels get 0.0
    for(size_t i = 0; i < robot->thrusterSetpoints_.size(); ++i)
    {
        const bool enabled = (i < msg->enables.size()) ? msg->enables[i] : true;
        robot->thrusterSetpoints_[i] = enabled ? static_cast<Scalar>(msg->omega_radps[i]) : Scalar(0);
    }
    robot->thrusterSetpointsChanged_ = true;
}
