...
// tauv message types used to remove adapter
...
bool ROS2ScenarioParser::ParseRobot(XMLElement* element)
{
    ...
    if((item = element->FirstChildElement("ros_subscriber")) != nullptr)
    {
        ...
        {
            // Subscribe to tauv_msgs/ThrusterSetpoint instead of Float64MultiArray
            std::function<void(const tauv_msgs::msg::ThrusterSetpoint::SharedPtr msg)> callbackFunc =
                        std::bind(&ROS2SimulationManager::ThrustersTauvCallback, sim, _1, rosRobot);
            subs[robot->getName() + "/thrusters"] = nh_->create_subscription<tauv_msgs::msg::ThrusterSetpoint>(std::string(topicThrust), 10, callbackFunc);
        }
        ...
        switch(sens->getType())
        {
            ...
            case SensorType::LINK:
            {
                switch(((ScalarSensor*)sens)->getScalarSensorType())
                {
                    ...
                    {
                        // Publish tauv_msgs/WaterlinkedDvlFrame directly
                        pubs[sensorName] = nh_->create_publisher<tauv_msgs::msg::WaterlinkedDvlFrame>(topicStr, queueSize);

                        // Optional altitude topic remains a Range message for compatibility
                    ...
                    case ScalarSensorType::PRESSURE:
                        // Publish tauv_msgs/DepthSensorFrame directly
                        pubs[sensorName] = nh_->create_publisher<tauv_msgs::msg::DepthSensorFrame>(topicStr, queueSize);
                        break;
