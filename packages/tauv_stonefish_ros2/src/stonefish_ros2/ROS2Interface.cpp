...
{
    // Declare parameters used by adapter parity
    // external_temperature: used for DepthSensorFrame (deg C)
    nh_->declare_parameter<double>("external_temperature", 25.0);
    nh_->get_parameter("external_temperature", external_temperature_);
}
...
{
    // Map Stonefish Pressure to tauv DepthSensorFrame
    Sample s = press->getLastSample();
    tauv_msgs::msg::DepthSensorFrame msg;
    msg.header.stamp = nh_->get_clock()->now();
    msg.header.frame_id = press->getName();

    // Adapter semantics:
    // - depth: not provided by sim sensor -> set to sentinel -1.0
    // - pressure: simulated pressure value from Stonefish
    // - temperature: configured external_temperature parameter
    msg.depth = -1.0;
    msg.pressure = s.getValue(0);
    msg.temperature = static_cast<float>(external_temperature_);

    std::static_pointer_cast<rclcpp::Publisher<tauv_msgs::msg::DepthSensorFrame>>(pub)->publish(msg);
}

void ROS2Interface::PublishDVL(rclcpp::PublisherBase::SharedPtr pub, DVL* dvl) const
{
    // Map Stonefish DVL to tauv WaterlinkedDvlFrame
    ...
    vVariance *= vVariance; // variance

    tauv_msgs::msg::WaterlinkedDvlFrame msg;
    msg.header.stamp = nh_->get_clock()->now();
    msg.header.frame_id = dvl->getName();

    // time: delta between consecutive frames for this DVL
    const std::string key = dvl->getName();
    auto it = last_dvl_stamp_.find(key);
    if (it != last_dvl_stamp_.end()) {
        msg.time = (msg.header.stamp - it->second).seconds();
    } else {
        msg.time = 0.0f;
    }
    last_dvl_stamp_[key] = msg.header.stamp;

    // time_of_transmission and time_of_validity in unix microseconds
    uint64_t sec = static_cast<uint64_t>(msg.header.stamp.seconds());
    uint64_t usec = static_cast<uint64_t>(msg.header.stamp.nanoseconds() / 1000);
    uint64_t unix_us = sec * static_cast<uint64_t>(1e6) + usec;
    // TODO(gleb): need better model for these
    msg.time_of_transmission = unix_us;
    msg.time_of_validity = unix_us;

    msg.altitude = (status == 0 || status == 2) ? static_cast<float>(s.getValue(3)) : -1.0f;

    msg.vx = static_cast<float>(s.getValue(0));
    msg.vy = static_cast<float>(s.getValue(1));
    msg.vz = static_cast<float>(s.getValue(2));
    msg.velocity_valid = true;

    // Fill diagonal covariance from Stonefish variance
    msg.covariance[0] = static_cast<float>(vVariance);
    msg.covariance[4] = static_cast<float>(vVariance);
    msg.covariance[8] = static_cast<float>(vVariance);

    msg.status = 0; // OK
    msg.fom = 0.0f; // Figure of merit not modelled

    std::static_pointer_cast<rclcpp::Publisher<tauv_msgs::msg::WaterlinkedDvlFrame>>(pub)->publish(msg);
}
