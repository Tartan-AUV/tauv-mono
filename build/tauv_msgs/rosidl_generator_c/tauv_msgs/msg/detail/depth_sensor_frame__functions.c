// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from tauv_msgs:msg/DepthSensorFrame.idl
// generated code does not contain a copyright notice
#include "tauv_msgs/msg/detail/depth_sensor_frame__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"

bool
tauv_msgs__msg__DepthSensorFrame__init(tauv_msgs__msg__DepthSensorFrame * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    tauv_msgs__msg__DepthSensorFrame__fini(msg);
    return false;
  }
  // depth
  // pressure
  // temperature
  return true;
}

void
tauv_msgs__msg__DepthSensorFrame__fini(tauv_msgs__msg__DepthSensorFrame * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // depth
  // pressure
  // temperature
}

bool
tauv_msgs__msg__DepthSensorFrame__are_equal(const tauv_msgs__msg__DepthSensorFrame * lhs, const tauv_msgs__msg__DepthSensorFrame * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // depth
  if (lhs->depth != rhs->depth) {
    return false;
  }
  // pressure
  if (lhs->pressure != rhs->pressure) {
    return false;
  }
  // temperature
  if (lhs->temperature != rhs->temperature) {
    return false;
  }
  return true;
}

bool
tauv_msgs__msg__DepthSensorFrame__copy(
  const tauv_msgs__msg__DepthSensorFrame * input,
  tauv_msgs__msg__DepthSensorFrame * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // depth
  output->depth = input->depth;
  // pressure
  output->pressure = input->pressure;
  // temperature
  output->temperature = input->temperature;
  return true;
}

tauv_msgs__msg__DepthSensorFrame *
tauv_msgs__msg__DepthSensorFrame__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tauv_msgs__msg__DepthSensorFrame * msg = (tauv_msgs__msg__DepthSensorFrame *)allocator.allocate(sizeof(tauv_msgs__msg__DepthSensorFrame), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(tauv_msgs__msg__DepthSensorFrame));
  bool success = tauv_msgs__msg__DepthSensorFrame__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
tauv_msgs__msg__DepthSensorFrame__destroy(tauv_msgs__msg__DepthSensorFrame * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    tauv_msgs__msg__DepthSensorFrame__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
tauv_msgs__msg__DepthSensorFrame__Sequence__init(tauv_msgs__msg__DepthSensorFrame__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tauv_msgs__msg__DepthSensorFrame * data = NULL;

  if (size) {
    data = (tauv_msgs__msg__DepthSensorFrame *)allocator.zero_allocate(size, sizeof(tauv_msgs__msg__DepthSensorFrame), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = tauv_msgs__msg__DepthSensorFrame__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        tauv_msgs__msg__DepthSensorFrame__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
tauv_msgs__msg__DepthSensorFrame__Sequence__fini(tauv_msgs__msg__DepthSensorFrame__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      tauv_msgs__msg__DepthSensorFrame__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

tauv_msgs__msg__DepthSensorFrame__Sequence *
tauv_msgs__msg__DepthSensorFrame__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tauv_msgs__msg__DepthSensorFrame__Sequence * array = (tauv_msgs__msg__DepthSensorFrame__Sequence *)allocator.allocate(sizeof(tauv_msgs__msg__DepthSensorFrame__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = tauv_msgs__msg__DepthSensorFrame__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
tauv_msgs__msg__DepthSensorFrame__Sequence__destroy(tauv_msgs__msg__DepthSensorFrame__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    tauv_msgs__msg__DepthSensorFrame__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
tauv_msgs__msg__DepthSensorFrame__Sequence__are_equal(const tauv_msgs__msg__DepthSensorFrame__Sequence * lhs, const tauv_msgs__msg__DepthSensorFrame__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!tauv_msgs__msg__DepthSensorFrame__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
tauv_msgs__msg__DepthSensorFrame__Sequence__copy(
  const tauv_msgs__msg__DepthSensorFrame__Sequence * input,
  tauv_msgs__msg__DepthSensorFrame__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(tauv_msgs__msg__DepthSensorFrame);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    tauv_msgs__msg__DepthSensorFrame * data =
      (tauv_msgs__msg__DepthSensorFrame *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tauv_msgs__msg__DepthSensorFrame__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tauv_msgs__msg__DepthSensorFrame__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!tauv_msgs__msg__DepthSensorFrame__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
