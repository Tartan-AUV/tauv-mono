...
ax.scatter(last_all['gain'], last_all['voltage'], last_all['rpm'],
           c=last_all['voltage'], cmap='plasma', s=15, alpha=0.8)
ax.set_xlabel('Gain')
...
ax  = fig.add_subplot(111, projection='3d')
ax.scatter(last_all['gain'], last_all['voltage'], last_all['rpm'],
           c=last_all['voltage'], cmap='plasma', s=10, alpha=0.4)

result = fit_surface_poly(last_all[last_all['gain'] >= 0], gain_sign=1)
if result:
    G, V, R = result
...
fig.add_trace(go.Scatter3d(
    ...
    z=last_all['rpm'],
    mode='markers',
    marker=dict(size=3, color=last_all['voltage'], colorscale='Plasma', opacity=0.6),
    name='settled points'
))
...
add_plotly_surface(last_all[last_all['gain'] >= 0], gain_sign=1,  color='royalblue', name='positive fit')
add_plotly_surface(last_all[last_all['gain'] <= 0], gain_sign=-1, color='salmon',    name='negative fit')
...
def remove_outliers(data, col='rpm', factor=1.5):
    q1 = data[col].quantile(0.25)
    q3 = data[col].quantile(0.75)
    iqr = q3 - q1
    return data[(data[col] >= q1 - factor * iqr) & (data[col] <= q3 + factor * iqr)]

clean = remove_outliers(last_all)
