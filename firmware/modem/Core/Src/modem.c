...
status_t demodulator_init(demod_t *m, const demodulator_config_t *c) {
    ...
    m->raw_prev_buf = c->combined_raw_buf;
    m->raw_last_buf = c->combined_raw_buf + c->chip_buf_size;
    m->raw_writing_buf = c->combined_raw_buf + 2 * c->chip_buf_size;
...
void cycle_raw_bufs(demod_t *m) {
    d_raw_t *t = m->raw_prev_buf;
    m->raw_prev_buf = m->raw_last_buf;
    m->raw_last_buf = m->raw_writing_buf;
    m->raw_writing_buf = t;
}
...
status_t demod_sdft(demod_t *m, d_sdft_t *dst, size_t dst_size) {
    ...
    if (dst_size < m->c.chip_buf_size) return MDM_ERROR;

    for (size_t i = 0; i < m->c.chip_buf_size; i++) {
        float sample = ((float) m->raw_last_buf[i]) / c->max_raw;
        ptrdiff_t sample_N_idx = i - c->chip_sdft_N;
        float sample_N;
        if (sample_N_idx < 0) {
            sample_N = (float) m->raw_prev_buf[m->c.chip_buf_size + sample_N_idx];
        } else {
            sample_N = (float) m->raw_last_buf[sample_N_idx];
        }
...
//
        ...
        HAL_DMAEx_MultiBufferStart_IT(hadc->DMA_Handle, (uint32_t)&hadc->Instance->DR, (uint32_t)dst0, (uint32_t)dst1, length);
