...
status_t demod_sdft(demod_t *m, d_sdft_t *dst, size_t dst_size) {
    ...
    for (size_t i = 0; i < m->c.chip_buf_size; i++) {
        float sample = normalize_sample(m, m->raw_last_buf[i]);
        ptrdiff_t sample_N_idx = i - c->chip_sdft_N;
        float sample_N;
        if (sample_N_idx < 0) {
            sample_N = normalize_sample(m, m->raw_prev_buf[m->c.chip_buf_size + sample_N_idx]);
        } else {
            sample_N = normalize_sample(m, m->raw_last_buf[sample_N_idx]);
        }
