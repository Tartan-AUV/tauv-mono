...
void Task_Eth100Hz(const Eth100HzInputMessage *inputMessage,
                   Eth100HzMessage *outputMessage) {
  // Payload construction
  const XsensIMUFrame *imuFrames = inputMessage->imuFrames;

  TAUV_FB_Eth100HzMsg_start_as_root(&builder);
  TAUV_FB_Eth100HzMsg_imu_data_start(&builder);
  for (size_t i = 0; i < inputMessage->nXsensImuFrames; ++i) {
    const XsensIMUFrame *imuFrame = &imuFrames[i];
    TAUV_FB_XsensIMUFrame_t imuFrameBuf;
    ...
    TAUV_FB_Eth100HzMsg_imu_data_push(&builder, &imuFrameBuf);
  }
  TAUV_FB_Eth100HzMsg_imu_data_end(&builder);
  TAUV_FB_Eth100HzMsg_end_as_root(&builder);
  size_t eth100HzMsgBufSize;
  const void *eth100HzMsgBuf =
      flatcc_builder_get_direct_buffer(&builder, &eth100HzMsgBufSize);
  assert(eth100HzMsgBuf);

  // transmit the buffer
  struct pbuf *p = pbuf_alloc(PBUF_TRANSPORT, eth100HzMsgBufSize, PBUF_RAM);
  memcpy(p->payload, eth100HzMsgBuf, eth100HzMsgBufSize);  // <-- this was missing too
