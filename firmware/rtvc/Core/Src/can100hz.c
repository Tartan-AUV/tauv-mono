...
#define XSENS_FRAME_COMPLETE				( XSENS_RECVD_SAMPLE_TIME \
											| XSENS_RECVD_ORIENTATION_QUATERNION \
											| XSENS_RECVD_RATE_OF_TURN \
											| XSENS_RECVD_FREE_ACCELERATION )
...
void Task_CAN100Hz(const CAN100HzInputMessage* inputMsg, CAN100HzMessage* outputMsg)
{
	...
	while (xQueueReceive(can100HzRxQueue, &canRxMsg, 0) == pdTRUE)
	{
		...
		const uint8_t devType = CAN_DEVICE_TYPE_FROM_ID(canMsgId);
		const uint8_t devId = CAN_DEVICE_ID_FROM_ID(canMsgId);
		const uint8_t msgType = CAN_MSG_TYPE_FROM_ID(canMsgId);

		switch (devType)
		{
		case CAN_DEVICE_TYPE_XSENS:
			if (XSENS_IS_IMU_MSG(msgType))
			{
				++canMsgCounter;
				assert(currImuMsgIdx < CAN_XSENS_IMU_FRAMES_MAX);

				switch (msgType) {
				case CAN_MSG_TYPE_XSENS_SAMPLE_TIME:
					...
					break;
				case CAN_MSG_TYPE_XSENS_ORIENTATION_QUATERNION:
					...
					imuFrames[currImuMsgIdx].Orientation = CAN_MsgParse_Xsens_Orientation(canRxMsg.data);
					break;
				case CAN_MSG_TYPE_XSENS_RATE_OF_TURN:
					...
					imuFrames[currImuMsgIdx].RateOfTurn = CAN_MsgParse_Xsens_RateOfTurn(canRxMsg.data);
					break;
				case CAN_MSG_TYPE_XSENS_FREE_ACCELERATION:
					...
					imuFrames[currImuMsgIdx].FreeAcceleration = CAN_MsgParse_Xsens_FreeAcceleration(canRxMsg.data);
					break;
				default:
					assert(false);
				}
	...
	outputMsg->NImuFrames = currImuMsgIdx;  // exclude the last frame which either empty or partially complete
}
