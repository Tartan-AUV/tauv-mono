...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<std::string>("error_topic", error_topic_from_plant_, "error");
  ...
  node_priv.param<bool>("use_error", listen_to_error_, false);
  ...
  ros::Subscriber error_sub_ = node.subscribe(error_topic_from_plant_, 1, &PidObject::plantErrorCallback, this);
  ...
  {
    while( ros::ok() && !ros::topic::waitForMessage<std_msgs::Float64>(error_topic_from_plant_, ros::Duration(10.)))
      ROS_WARN_STREAM("Waiting for first error message.");
  }
...
void PidObject::plantErrorCallback(const std_msgs::Float64& error_msg)
{
  plant_error_ = error_msg.data;
...
void PidObject::printParameters()
{
  ...
  std::cout << "Name of error topic: " << error_topic_from_plant_ << std::endl;
  std::cout << "User error: " << listen_to_error_ << std::endl;
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
                         ...
                         (ros::Time::now() - last_setpoint_msg_time_).toSec() <= setpoint_timeout_))
    {
      ...
      // Publish topic with
      std::vector<double> pid_debug_vect { plant_error_, control_effort_, proportional_, integral_, derivative_};
