...
class OAKDNode:
    def __init__(self):
        ...
        self.left.setResolution(depthai.MonoCameraProperties.SensorResolution.THE_720_P)
        self.left.setBoardSocket(depthai.CameraBoardSocket.LEFT)

        self.right.setResolution(depthai.MonoCameraProperties.SensorResolution.THE_720_P)
        self.right.setBoardSocket(depthai.CameraBoardSocket.RIGHT)
        ...
        self.depthPub = rospy.Publisher("/oakd/oakd_front/depth_map", Image, queue_size=QUEUE_SIZE)
        self.colorPub = rospy.Publisher("/oakd/oakd_front/color_image", Image, queue_size=QUEUE_SIZE)
        #TODO: change to service
        self.cameraInfoPub = rospy.Publisher("/oakd/oakd_front/camera_info", CameraInfo, queue_size=QUEUE_SIZE)

        self.device = depthai.Device(self.pipeline)
        
        self.calibData = self.device.readCalibration()
        self.camera_info = CameraInfo()
        self.camera_info.K = np.ndarray.flatten(np.array(self.calibData.getCameraIntrinsics(depthai.CameraBoardSocket.RGB)))
        self.camera_info.distortion_model = "rational_polynomial"
        self.camera_info.D = np.array(self.calibData.getDistortionCoefficients(depthai.CameraBoardSocket.RGB))
    ...
    def spin(self):
        ...
        while True:
            ...
            if rgb is not None:
                try:
                    img = self.bridge.cv2_to_imgmsg(rgb.getCvFrame(), "rgb8")
                    img.header.frame_id = "oakd_front"
                    self.colorPub.publish(img)
                except CvBridgeError as e:
                    rospy.loginfo("OAKD frame error")

            if depth is not None:
                try:
                    img = self.bridge.cv2_to_imgmsg(depth.getCvFrame(), "passthrough")
                    img.header.frame_id = "oakd_front"
                    self.depthPub.publish(img)
                except CvBridgeError as e:
                    rospy.loginfo("OAKD frame error")
