...
class TestThrusterManagerProportionalCorrect(unittest.TestCase):
    ...
    def test_thrusters(self):
        self.assertEqual(len(MANAGER.thrusters_sim), 3)

        for i in range(len(MANAGER.thrusters_sim)):
            self.assertEqual(MANAGER.thrusters_sim[i].index, i)
            self.assertEqual(MANAGER.thrusters_sim[i].topic, 'thrusters/{}/input'.format(i))
            self.assertEqual(MANAGER.thrusters_sim[i].LABEL, 'proportional')
            self.assertTrue(np.isclose(REFERENCE_TAM[:, i].flatten(), MANAGER.thrusters_sim[i].tam_column).all())
